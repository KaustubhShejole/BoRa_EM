...
Runs the HBTL, PG-EM, CrowdBT, and NoisyBT ablations over a grid of epsilon
...
def results_to_json_safe(d):
    ...
    safe = {}
    for k, v in d.items():
        key = str(k)  # JSON object keys must be strings (epsilons are floats)
...
# Ablation runners
# ---------------------------------------------------------------------------
def run_htcv(PC_faceage, df_faceage, device, epsilons, lr, max_iter):
    """### HBTL"""
    HTCV = defaultdict(list)
    gradem_time = []
    for sd in range(1):
        for epsilon in epsilons:
            start = time.time()
            grad_em = HTCVWrapper(PC_faceage, lr, sd, device, max_iter=max_iter, tol=epsilon)
            ...
            HTCV[epsilon] = [grad_acc, grad_wacc, grad_tau]
            print(HTCV[epsilon])

    print(HTCV)
    return HTCV, gradem_time

def run_hbtl(PC_faceage, df_faceage, device, epsilons, lr, max_iter):
    """### HBTL"""
    ...
    for sd in range(1):
        for epsilon in epsilons:
            start = time.time()
            grad_em = GradientEMWrapper(PC_faceage, lr, sd, device, max_iter=max_iter, tol=epsilon)
            r_est, beta_est = grad_em.run_algorithm()
            end = time.time()
            gradem_time.append(end - start)
            ...
            HBTL[epsilon] = [grad_acc, grad_wacc, grad_tau]
            print(HBTL[epsilon])
...
def run_pgem(PC_faceage, df_faceage, device, epsilons, max_iter):
    """### PG EM"""
    ...
    for sd in range(1):
        for epsilon in epsilons:
            start = time.time()
            pg = EMWrapper(PC_faceage, max_iter, device, sd, tol=epsilon)
            ...
            PGEM[epsilon] = [pgem_acc, pgem_wacc, pgem_tau]
            print(PGEM[epsilon])
...
def run_crowdbt(PC_faceage, df_faceage, device, epsilons, size, num_reviewers, lr, tol, max_iter):
    """### CrowdBT"""
    gt_scores = to_numpy(df_faceage["score"].tolist())
        ...
        try:
            for epsilon in epsilons:
                ...
                crowdbt_test = opt_fair.CrowdBT_3_0(
                    data=PC_faceage, penalty=0, device=device, random_seed=seed,
                )
                crowdbt_scores, _ = crowdbt_test.alternate_optim(
                    size, K, lr_x=lr, lr_y=lr, iters=max_iter, verbose=False, tol=epsilon,
                )
                end = time.time()
                ...
                CrowdBT[epsilon] = [crowdbt_acc, crowdbt_wacc, crowdbt_tau]
                print(CrowdBT[epsilon])
...
def run_noisybt(df_faceage, device, epsilons, size, num_reviewers, lr, tol, max_iter):
    """### FactorBT (NoisyBT)"""
        ...
        try:
            for epsilon in epsilons:
                ...
                noisybt_test = opt_fair.NoisyBT_3_0(
                    data=PC_faceage_noisybt, penalty=0, device=device, random_seed=seed,
                )
                ...
                NoisyBT[epsilon] = [noisybt_acc, noisybt_wacc, noisybt_tau]
        except Exception as e:
            print(e)
...
def main():
    ...
    # --- HBTL ---
#     HBTL, gradem_time = run_hbtl(PC_faceage, df_faceage, device, epsilons, lr, max_iter)
    
    HTCV, htcv_time = run_htcv(PC_faceage, df_faceage, device, epsilons, lr, max_iter)

#     # --- PG EM ---
#     PGEM, pgem_time = run_pgem(PC_faceage, df_faceage, device, epsilons, max_iter)
...
#         PC_faceage, df_faceage, device, epsilons, size, num_reviewers, lr, tol, max_iter
#     )

#     NoisyBT, noisybt_time = run_noisybt(
#         df_faceage, device, epsilons, size, num_reviewers, lr, tol, max_iter
#     )

    # --- Save everything to a single JSON file ---
    output = {
        "HTCV": results_to_json_safe(HTCV),
...
#             "noisybt_time": noisybt_time,
    ...
    out_path = "ablation_results_epsilon_htcv.json"
